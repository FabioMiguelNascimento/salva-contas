...
const getFriendlyAiAdvisorErrorMessage = (error: unknown): string => {
  ...
  if (code === 'AI_RATE_LIMIT') {
    return 'O assistente esta com muitas requisicoes agora. Tente novamente em alguns instantes.';
  }

  if (code === 'AI_UNSUPPORTED_INPUT') {
    return 'Esse tipo de arquivo ainda nao e suportado para processamento pela IA.';
  }
  ...
  if (message.includes('json inválido') || message.includes('malformado')) {
    return 'A resposta da IA veio em formato invalido. Tente novamente em instantes.';
  }

  if (message.includes('arquivo nao encontrado')) {
    return 'O arquivo anexado nao foi encontrado no processamento. Reanexe e tente novamente.';
  }
